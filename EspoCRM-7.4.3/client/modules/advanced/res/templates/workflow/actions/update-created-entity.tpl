...
<div class="row">
    ...
    <div class="col-md-10">
        {{translate actionType scope='Workflow' category='actionTypes'}}{{#if entityTypeTranslated}}{{#if linkTranslated}} <span class="text-muted">&raquo;</span> {{linkTranslated}}{{/if}} <span class="text-muted">&raquo;</span> {{{entityTypeTranslated}}}{{/if}}
        {{#if text}}
            '{{text}}'
        {{else}}
