...
class CaseByLastActivity extends Base
{
    protected $rangeList = [
        [0, 7],
        [7, 15],
    ...
    protected function executeSubReport($where, $params)
    {
        ...
        $selectManager = $this->getSelectManagerFactory()->create('Case');
        ...
        $collection = $this->getEntityManager()->getRepository('Case')->find($selectParams);
        $count = $this->getEntityManager()->getRepository('Case')->count($selectParams);
    ...
    public function run($where = null, array $params = null)
    {
        ...
        $statusList = $this->getMetadata()->get('entityDefs.Case.fields.status.options', []);
        ...
        foreach ($grouping[1] as $status) {
            $groupValueMap['status'][$status] = $this->getLanguage()->translateOption($status, 'status', 'Case');
        }
        ...
        $sql = "
                (
                    (
                        ...
                        FROM `call`
                        INNER JOIN call_case ON `call`.id = call_case.call_id AND call_case.deleted=0
                        WHERE call_case.case_id = case.id AND `call`.status = 'Held' AND `call`.deleted=0
                        UNION
                        SELECT MAX(meeting.date_start) AS 'maxDate'
                        FROM `meeting`
                        INNER JOIN case_meeting ON meeting.id = case_meeting.meeting_id AND case_meeting.deleted=0
                        WHERE case_meeting.case_id = case.id AND meeting.status = 'Held' AND meeting.deleted=0
                        ORDER BY `maxDate` DESC
                        LIMIT 1
                    ) {$rangePart}
                ) AND
                case.status NOT IN ('".implode("', '", $this->ignoredStatusList)."')
        ";
    ...
    protected function getDataResults()
    {
        ...
        foreach ($rangeList as $i => $range) {
            ...
            $sql = "
                SELECT COUNT(case.id) AS 'COUNT:id', `case`.status AS 'status'
                FROM `case`
                WHERE
                    `case`.deleted = 0 AND
                    {$wherePart}
